        ...
        <Animated.View style={[styles.container, { opacity, padding: styleSize === 'large' ? 7 : styleSize === 'medium' ? 14 : 21 }]}>
            {
                ...
                <View style={{ width: '100%',height: 1, flexDirection }}>
                    ...
                    >
                        <TextInput editable={false} style={{ position: 'absolute', width: 40, textAlign: 'center', ...svgOffset, color: valueLabelsTextColor, bottom: 25, fontWeight: 'normal' }} ref={fromValueTextRef} />
                    </Animated.View>
                    <Animated.View
                        style={{ position: 'absolute', bottom: 0, left: 0, alignItems: 'center', transform: [{ translateX: translateXtoValue }, { scale: toValueScale }] }}
                    >
                        <TextInput editable={false} style={{ position: 'absolute', width: 40, textAlign: 'center', ...svgOffset, color: valueLabelsTextColor, bottom: 25, fontWeight: 'normal' }} ref={toValueTextRef} />
                    </Animated.View>
                </View>
            }
            <View style={{ width: '100%', height: knobSize, marginVertical: 4, position: 'relative', flexDirection, alignItems: 'center' }}>
                ...
                <PanGestureHandler onGestureEvent={onGestureEventFromValue} onHandlerStateChange={onHandlerStateChangeFromValue}>
                  {/* Left knob */}
                    <Animated.View style={[styles.knob, { height: knobSize, width: knobSize, borderRadius: knobSize, borderColor: '#FFFFFF', borderWidth: 2, backgroundColor: fromKnobColor, elevation: fromElevation, transform: [{ translateX: translateXfromValue }] }]} >
                      <TextInput editable={false} style={{ position: 'absolute', width: 50, textAlign: 'left', ...svgOffset, color: valueLabelsTextColor, top: 25, fontWeight: 'normal' }} ref={fromValueTextRef} />
                    </Animated.View>
                </PanGestureHandler>
                <PanGestureHandler onGestureEvent={onGestureEventToValue} onHandlerStateChange={onHandlerStateChangeToValue}>
                  {/* Right knob */}
                    <Animated.View style={[styles.knob, { height: knobSize, width: knobSize, borderRadius: knobSize, borderColor: '#FFFFFF', borderWidth: 2, backgroundColor: toKnobColor, elevation: toElevation, transform: [{ translateX: translateXtoValue }] }]} >
                      <TextInput editable={false} style={{ position: 'absolute', width: 50, textAlign: 'left', ...svgOffset, color: valueLabelsTextColor, top: 25, fontWeight: 'normal' }} ref={toValueTextRef} />
                    </Animated.View>
                </PanGestureHandler>
            </View>
            {
                ...
                <View style={{ width: '100%', flexDirection, justifyContent: 'space-between' }}>
                    <Text style={{ color: rangeLabelsTextColor, fontWeight: "normal", fontSize }}>{min}</Text>
                    <Text style={{ color: rangeLabelsTextColor, fontWeight: "normal", fontSize }}>{max}</Text>
                </View>
            }
        </Animated.View>
    );
